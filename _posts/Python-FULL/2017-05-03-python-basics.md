...
|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
|**[python 诞生](#z1)**|简单入门|指定编码|注释|表达式|输入|help|ID
|**[变量](#z2)**|可变不可变类型|动态类型|赋值|


<h3 id='z1'>一 Python 的诞生</h3>
...
1+2 就是一个表达式，这里的加号叫做运算符，1、2叫做操作数。  
1+2 经过计算后得到的结果是3，就1+2 = 3。  
我们可以将计算结果保存在一个变量里，ret = 1+2 。  
**所以表达式就是由操作数和运算符组成的一句代码或语句，表达式可以求值，可以放在“=”的右边，用来给变量赋值。**

#### 5、Linux/Unix 用户需要注意的内容
...
Shebang 是一个由 “#”  和 “!” 构成的字符串行（#!），她出现在文件的第一行。当文件中出现 Shebang 时，Linux/Unix 操作系统的的程序载入器会分析 Shebang 的内容，将之后的内容作为解释器命令，并调用该执行，将载有 Shebang 的文件路径作为解释器的参数。  
